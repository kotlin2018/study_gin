...
type FooStruct struct {
	Foo string `msg_pack:"foo" json:"foo" form:"foo" xml:"foo" validator:"required"`
}

type FooBarStruct struct {
	FooStruct
	Bar string `msg_pack:"bar" json:"bar" form:"bar" xml:"bar" validator:"required"`
}

type FooBarFileStruct struct {
	FooBarStruct
	File *multipart.FileHeader `form:"file" validator:"required"`
}

type FooBarFileFailStruct struct {
	FooBarStruct
	File *multipart.FileHeader `invalid_name:"file" validator:"required"`
	// for unexport test
	data *multipart.FileHeader `form:"data" validator:"required"`
}

type FooDefaultBarStruct struct {
	FooStruct
	Bar string `msg_pack:"bar" json:"bar" form:"bar,default=hello" xml:"bar" validator:"required"`
}

type FooStructUseNumber struct {
	Foo interface{} `json:"foo" validator:"required"`
}
...
type FooStructForStringPtrType struct {
	...
	PtrBar *string `form:"ptr_bar" validator:"required"`
}
...
func TestBindingDefault(t *testing.T) {
	...
	assert.Equal(t, Form, Default("GET", HttpJSON))

	assert.Equal(t, JSON, Default("POST", HttpJSON))
	assert.Equal(t, JSON, Default("PUT", HttpJSON))

	assert.Equal(t, XML, Default("POST", HttpXML))
	assert.Equal(t, XML, Default("PUT", HttpXML2))

	assert.Equal(t, Form, Default("POST",HttpPOSTForm ))
	assert.Equal(t, Form, Default("PUT", HttpPOSTForm))

	assert.Equal(t, FormMultipart, Default("POST", HttpMultipartPOSTForm))
	assert.Equal(t, FormMultipart, Default("PUT", HttpMultipartPOSTForm))

	assert.Equal(t, ProtoBuf, Default("POST", HttpProtoBuf))
	assert.Equal(t, ProtoBuf, Default("PUT", HttpProtoBuf))

	assert.Equal(t, YAML, Default("POST", HttpYAML))
	assert.Equal(t, YAML, Default("PUT", HttpYAML))
}
...
func TestBindingFormStringSliceMap(t *testing.T) {
	...
	req.Header.Add("Content-Type", HttpPOSTForm)
...
func createFormPostRequest(t *testing.T) *http.Request {
	...
	assert.NoError(t, err)
	req.Header.Set("Content-Type", HttpPOSTForm)
	return req
}
...
func createFormFilesMultipartRequest(t *testing.T) *http.Request {
	...
	req.Header.Set("Content-Type", HttpMultipartPOSTForm+"; boundary="+boundary)
...
func TestFormBindingMultipartFail(t *testing.T) {
	...
	assert.NoError(t, err)
	req.Header.Set("Content-Type", HttpMultipartPOSTForm+";boundary=testboundary")
...
func testProtoBodyBinding(t *testing.T, b Binding, name, path, badPath, body, badBody string) {
	...
	req := requestWithBody("POST", path, body)
	req.Header.Add("Content-Type", HttpProtoBuf)
	err := b.Bind(req, &obj)
	assert.NoError(t, err)
